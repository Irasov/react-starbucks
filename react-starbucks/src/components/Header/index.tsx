...
const Header: React.FC = () => {
  ...
  React.useEffect(() => {
    if (isMounted.current) {
      ...
      localStorage.setItem('coffee', json);
    }
            ...
            <div className={styles.menu}>
              ...
              <Link to="/shop" className={styles.item} onClick={active ? iconClick : undefined}>Shop</Link>
              {
                location.pathname !=='/cart' && (
                  <Link to="/cart" className={styles.item} onClick={active ? iconClick : undefined}>
                    <div className={styles.btn}>
                      <span className={styles.btn__sum}>
                        {totalPrice.toFixed(2)} $   
                      </span>
                      <div className={styles.btn__delimetr}>
                      </div>
                      <svg
                        width="18"
                        height="18"
